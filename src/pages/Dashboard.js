...
function CertificateVerification() {
  ...
  async function handleVerify() {
    ...
    setCertificate(null);
    try {
      const res = await fetch(`${process.env.REACT_APP_API_BASE || 'http://localhost:5000'}/api/courses/verify-certificate/${searchId}`);
      if (!res.ok) {
    ...
    <div className="card" style={{ padding: 14, marginTop: 16 }}>
      ...
      <div style={{ marginTop: 8 }}>
        <label>Search ID:
          <input
            ...
            placeholder="Enter certificate search ID"
          />
        </label>
        <button className="btn btn-primary" onClick={handleVerify} style={{ marginLeft: 8 }}>Verify</button>
      </div>
      ...
      {certificate && (
        <div style={{ marginTop: 16 }}>
          ...
          <p><strong>Serving Event:</strong> {certificate.servingEvent}</p>
          <p><strong>Code:</strong> {certificate.code}</p>
          <p><strong>Class:</strong> {certificate.class}</p>
          <p><strong>HSC Batch:</strong> {certificate.hscBatch}</p>
        </div>
      )}
    </div>
  );
