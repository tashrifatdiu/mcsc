...
export async function adminFetchPendingJournals(adminToken) {
  const headers = adminToken ? { Authorization: `Bearer ${adminToken}` } : {};
  const res = await fetch(`${API_BASE.replace(/\/$/, '')}/api/admin/journals/pending`, { headers });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Failed to fetch pending journals');
  return data;
}

export async function adminFetchJournalById(id, adminToken) {
  const headers = adminToken ? { Authorization: `Bearer ${adminToken}` } : {};
  const res = await fetch(`${API_BASE.replace(/\/$/, '')}/api/admin/journals/${id}`, { headers });
  const data = await res.json().catch(() => null);
  if (!res.ok) throw new Error(data?.error || 'Failed to fetch journal');
  return data;
}

export async function adminApproveJournal(id, adminToken) {
  const headers = adminToken ? { Authorization: `Bearer ${adminToken}` } : {};
  const res = await fetch(`${API_BASE.replace(/\/$/, '')}/api/admin/journals/${id}/approve`, {
    method: 'PATCH',
    headers
  });
  const data = await res.json();
...
export async function adminDeleteJournal(id, adminToken) {
  const headers = adminToken ? { Authorization: `Bearer ${adminToken}` } : {};
  const res = await fetch(`${API_BASE.replace(/\/$/, '')}/api/admin/journals/${id}`, {
    method: 'DELETE',
    headers
  });
  const data = await res.json().catch(() => null);
...
export async function fetchCourses() {
  const res = await fetch(`${API_BASE.replace(/\/$/, '')}/api/courses`);
  const data = await res.json().catch(() => null);
...
export async function fetchCourseById(id) {
  const res = await fetch(`${API_BASE.replace(/\/$/, '')}/api/courses/${id}`);
  const data = await res.json().catch(() => null);
...
export async function adminDeleteCourse(id) {
  const res = await fetch(`${API_BASE.replace(/\/$/, '')}/api/courses/${id}`, { method: 'DELETE' });
  const data = await res.json().catch(() => null);
  if (!res.ok) throw new Error(data?.error || 'Delete failed');
  return data;
}